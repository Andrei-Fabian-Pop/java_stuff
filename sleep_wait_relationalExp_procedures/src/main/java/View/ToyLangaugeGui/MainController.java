...
public class MainController {
    public TableView<Pair<String, String>> ProcTable;
    public TableColumn<Pair<String, String>, String> ProcTableC1;
    public TableColumn<Pair<String, String>, String> ProcTableC2;
    ...
    public void initialize() {
        ...
        this.ProcTableC1.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().first));
        this.ProcTableC2.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().second));
    ...
    private void populateSymbolTable() {
        ...
        if (programState != null && !programState.getSymTable().isEmpty()) {
            for (Map.Entry<String, Values> entry : programState.getSymTable().peek().getContent().entrySet()) {
    ...
    private void populateProcTable() {
        ...
        if (!controller.getProgramState().isEmpty()) {
            procedureTable = controller.getProgramState().get(0).getProcedureTable();
        } else {
            procedureTable = new MyProcedureTable();
        }

        List<Pair<String, String>> procedureTableList = new ArrayList<>();
        for (Map.Entry<String, Pair<List<String>, IStatement>> entry : procedureTable.getProcedureTable().entrySet()) {
            procedureTableList.add(new Pair<>(entry.getKey() + entry.getValue().first.toString().replace('[', '(').replace(']', ')'), entry.getValue().second.toString()));
        }
        this.ProcTable.setItems(FXCollections.observableList(procedureTableList));
