...
public class ProgramState {
    ...
    private final Stack<IDictionary<String, Values>> symTable;
    ...
    public ProgramState(IStatement originalProgram) {
        ...
        symTable = new Stack<>(); symTable.push(new MyDictionary<>());
        out = new MyList<>();
    ...
    public ProgramState(IStack<IStatement> execStack, Stack<IDictionary<String, Values>> symTable, IList<String> out, IDictionary<String, BufferedReader> fileTable, IHeap heap, IProcedureTable procedureTable) {
    ...
    public Stack<IDictionary<String, Values>> getSymTable() {
        return symTable;
    }
    ...
    protected IStatement loadForkCompStatements(IStatement statement) {
        while (statement instanceof CompStatement) {
            statement.execute(this);
            statement = execStack.pop();
        }
    ...
    public ProgramState oneStep() throws MyException {
        ...
        if (statement instanceof CompStatement) {
            statement = loadForkCompStatements(statement);
        }
    ...
    private static int newId() {
        ...
        synchronized (ids) {
            do {
                _id = 1000 + (abs(random.nextInt()) % 9000); // to look like a pid :))
