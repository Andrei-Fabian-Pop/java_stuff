        ...
        while (!programStatesList.isEmpty()) {
            ...
            state.getHeap().setContent(
                    safeGarbageCollector(
                            getAddrFromSymTable(
                                    programStatesList.stream().map(programState -> programState.getSymTable().peek().getContent().values()).collect(Collectors.toList()),
...
//        repository.setProgramStates(programStatesList);
    ...
    public void oneStepAllGui() throws MyException {
//        typeChecker();
        ...
        state.getHeap().setContent(
                safeGarbageCollector(
                        getAddrFromSymTable(
                                programStateList.stream().map(programState -> programState.getSymTable().peek().getContent().values()).collect(Collectors.toList()),
                                state.getHeap().getContent()
                        ),
                        state.getHeap().getContent()
                )
