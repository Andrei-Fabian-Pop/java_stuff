...
public class MainController {
    public TableView<Pair<Integer, Integer>> latchTable;
    public TableColumn<Pair<Integer, Integer>, String> LatchTableC1;
    public TableColumn<Pair<Integer, Integer>, String> LatchTableC2;
    ...
    public void initialize() {
        ...
        this.LatchTableC1.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().first.toString()));
        this.LatchTableC2.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().second.toString()));
    ...
    private void populateLatchTable() {
        ILatch latch;
        if (!controller.getProgramState().isEmpty()) {
            latch = controller.getProgramState().get(0).getLatchTable();
        } else {
            latch = new MyLatch();
        }

        List<Pair<Integer, Integer>> latchList = new ArrayList<>();
        for (Map.Entry<Integer, Integer> entry : latch.getLatchTable().entrySet()) {
